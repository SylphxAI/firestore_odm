...
void main() {
  ...
  setUp(() async {
    ...
    db = odm;
    for (var i = 1; i <= 10; i++) {
      await odm.users.set(sampleUser(id: 'u${i.toString().padLeft(2, '0')}', age: i));
    }
  });
  ...
  test('orderBy ascending/descending with nested fields', () async {
    final asc = await db.users
        ...
        .get();
    expect(asc.first.profile.followers, greaterThan(asc.last.profile.followers));
  });

  test('object cursor pagination pages through without skips or duplicates',
      () async {
  ...
  test('endBefore / endAt', () async {
    final before = await db.users
        .orderBy(($) => ($.age(),))
        .endBefore((4,))
        .get();
    expect(before.map((u) => u.age), [1, 2, 3]);
    final at = await db.users
        .orderBy(($) => ($.age(),))
        .endAt((4,))
        .get();
    expect(at.length, 4);
